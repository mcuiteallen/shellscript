...
function startinfactory() {
	...
	sleep 3	
	sudo systemctl start start-infactory-frontend
	sleep 3	
	sudo systemctl start start_AuthCenter	
}
...
function startinpark() {
	...
	sudo systemctl start start_edge
	sleep 3
    sudo systemctl start start_inpark
    sleep 3
	...
	sleep 3
	sudo mv /opt/Tomcat/tomcatINFACTORY-FRONTEND /opt/BAK	

	sudo systemctl disable start_AuthCenter
	sleep 3	
	sudo mv /opt/AUTHCENTER /opt/BAK	
	
	sudo systemctl disable start-infactory-frontend
	sleep 3
	sudo mv /opt/Tomcat/tomcatINFACTORY-FRONTEND /opt/BAK	
}
...
function startat() {
	...
	sudo systemctl start start_cserver
	sleep 3
	sudo systemctl start start_ectuary
	sleep 3
}
